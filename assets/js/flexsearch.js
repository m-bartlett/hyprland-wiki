...
// This file overrides the Hextra theme's assets/js/flexsearch.js to:
//   - List all matching pages (high default caps, configurable via config.toml)
//   - Show breadcrumb + matching subsection for every result (grouped by page)
...
(function () {
  ...
  function search(e) {
    ...
    // Configurable search limits — defaults are intentionally high so all matches are shown.
    // Override via config.toml: [params.search.flexsearch] maxPageResults / maxSectionResults
    const maxPageResults = parseInt('{{- site.Params.search.flexsearch.maxPageResults | default 100 -}}', 10);
    const maxSectionResults = parseInt('{{- site.Params.search.flexsearch.maxSectionResults | default 100 -}}', 10);
    ...
    for (let i = 0; i < pageResults.length; i++) {
      ...
      for (let j = 0; j < nResults; j++) {
        ...
        results.push({
          ...
          _section_rk: j,
          route: url,
          // Always include the page breadcrumb so every result carries its page context.
   ...
   * Results from the same page are visually grouped: the page breadcrumb is shown
   * as a section header and only repeated when the page changes, so every result
   * row is always visually associated with its page context.
   *
  ...
  function displayResults(results, query) {
    ...
    // Track the last prefix rendered so we only show it once per page group.
    let lastPrefix = null;
    for (let i = 0; i < results.length; i++) {
      const result = results[i];
      // Render a page breadcrumb group header whenever the page changes.
      if (result.prefix && result.prefix !== lastPrefix) {
